...
- **Subagent dispatches (SDD implementers, reviewers, fixers) are BANNED on Anthropic models.** The db-backed-skills final review on `claude-fable-5.1` burned $7.94 against a $7.25 whole-plan budget — an order of magnitude over a comparable OpenRouter dispatch. Pick non-Anthropic ids from `agent_models` (`sort=code`, skip anything containing `anthropic`/`claude`): `glm-5.3` is the mid-tier workhorse, `gpt-5.6-sol`/`grok-4.6` when a task needs more capability. Anthropic ids also draw down the personal subscription's 5h/7d rate-limit windows (`quota_status`), which is a second reason they are never the default.
