...
- **Subagent dispatches (SDD implementers, reviewers, fixers) are BANNED on Anthropic models.** The db-backed-skills final review on `claude-fable-5.1` burned $7.94 against a $7.25 whole-plan budget — an order of magnitude over a comparable OpenRouter dispatch (and Anthropic ids also draw down the personal subscription's 5h/7d windows — `quota_status`). Pick non-Anthropic ids from `agent_models` (`sort=code`, skip anything containing `anthropic`/`claude`). Error on the side of cheaper models. Only pick models > $1/mtok when told to. z-ai/glm-5.3-flash should work for most things — it completed a full fix wave; **avoid deepseek-v4-flash, it cannot reliably make tool calls.** Reviewers sit one tier ABOVE what they review — same-model review is correlation, not verification, since it shares the implementer's blind spots. The one whole-branch final review takes `google/gemini-3.8-flash` (code 76.3, $0.75/$3.75 — near-frontier capability at input prices, which is what a review consumes; the output side breaks the $1 line and is the sanctioned exception). Escalation for a stuck implementer (fix rounds 4–5): `openai/gpt-5.6-sol` or `x-ai/grok-4.6` (code 77.4/76.8).
