...
- **`proxyenv.Claude`'s returned args carry `--mcp-config=<json>` whenever a URL is set — gated on URL, never on Model — and `RAFIKI_MCP_TOKEN` is a `Managed` var.** The MCP config points a server named `rafiki` at `<ClaudeOptions.URL>/mcp` with `Authorization: Bearer ${RAFIKI_MCP_TOKEN}` — a placeholder Claude Code expands at connect time, never the token inline (argv is world-readable via `ps`); `Managed` membership strips any inherited copy before the child's value is set, so a nested launch cannot adopt the outer session's bearer. Gating the append on `o.Model` instead would silently strip agent control from a bare `rafiki claude`. Two drift traps: the `/mcp` path literal exists in BOTH `pkg/proxyenv` (`mcpPath`) and `cmd/rafikid/mcp_face.go` (`mcpFacePath`) — grep the other side before changing either (proxyenv is a leaf package and cannot import the main package); and `pkg/daraja`'s suppression of the plain `--model` keys on a `--model` PAIR being present in `ProxyModelArgs` (`slices.Contains`), not on non-nil — args now carry unrelated argv (`--mcp-config`) with no model attached, and the non-nil form would drop a caller-supplied model. Callers that forward `args` to argv (interactive `rafiki claude`, daraja) get injection for free; the daemon's local-spawn path takes the same return through `buildEnv`'s `extraArgv`. `TestMCPInjectedConfigReachesTwelveTools` (`test/integration`) pins the seam end to end against a real daemon.
