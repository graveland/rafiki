...
- **`go test -run` is an UNANCHORED substring match, so a verify command silently skips tests its pattern does not prefix.** The conversation-review plan's verify (`-run TestReview`) never matched its own pinned `TestConversationReview*` names — five of eight required cases passed while the clamp/scope/model fast-fail tests never ran. When a plan (or a task's verify line) pins both a `-run` pattern and test names, check every required name actually contains the pattern; when the two pins conflict and neither may change, a TestPattern-prefixed shim whose subtests call the pinned-named test bodies (`t.Run("…", TestPinnedName)`) satisfies both, at the cost of running each body twice under a full `-run .` suite (inert for independent bodies).
...
- **`_CLAUDE_CODE_ASSUME_FIRST_PARTY_BASE_URL=1` also silently re-enables deferred tools (tool search), which 400s every non-Anthropic model.** Claude Code gates the byte watchdog *and* tool search on the same `Yd()` predicate (v2.1.220 `s3()`: `!ENABLE_TOOL_SEARCH && provider==="firstParty" && !Yd()`), so the watchdog fix in `pkg/proxyenv.Defaults` costs you the guard that would otherwise turn tool search off behind a custom base URL. With it on, Claude Code omits tools from `tools[]` and sends `tool_reference` blocks, and the first turn on an OpenRouter-routed model returns `400 Deferred custom tools are only supported on Anthropic models`. `proxyenv.ClaudeEnv` sets `ENABLE_TOOL_SEARCH=false` when `anthropicModel(o.Model)` is false. That predicate is a string-shape test, not a catalog lookup — a new *Anthropic* id spelling that neither starts with `claude` nor ends in `-latest` would wrongly lose tool search (harmless, costs tokens); a non-Anthropic id that sneaks past it breaks the session outright.

- **`proxyenv.ClaudeEnv` is the only launch-decision entry point: it returns `(env []string, Values)` where `Values.MCPConfig` is the BARE inline JSON and `Values.ModelArgs` is the custom-model-option `--model` pair, and `RAFIKI_MCP_TOKEN` is a `Managed` var.** `claudeargv.Build` renders `Values` into argv itself — `Params.MCPConfig` is prefixed with `--mcp-config=` by `Build`, so `Values.MCPConfig` must never arrive as a full element (a producer handing the rendered element back produces `--mcp-config=--mcp-config={...}`; `TestBuildClaudeArgvCarriesMCPConfig` pins the single-element + JSON-valid shape). The MCP config points a server named `rafiki` at `<ClaudeOptions.URL>/mcp` with `Authorization: Bearer ${RAFIKI_MCP_TOKEN}` — a placeholder Claude Code expands at connect time, never the token inline (argv is world-readable via `ps`); `Managed` membership strips any inherited copy before the child's value is set, so a nested launch cannot adopt the outer session's bearer. Gating the append on `o.Model` instead would silently strip agent control from a bare `rafiki claude`. Two drift traps: the `/mcp` path literal exists in BOTH `pkg/proxyenv` (`mcpPath`) and `cmd/rafikid/mcp_face.go` (`mcpFacePath`) — grep the other side before changing either (proxyenv is a leaf package and cannot import the main package); and `claudeargv.Params.ModelArgs` suppresses the plain `--model` pair when NON-EMPTY, not when it merely contains `--model` — `ModelArgs` is nil or exactly the pair, so both conventions coincide today, but a future caller passing unrelated argv into `ModelArgs` expecting pair-positive semantics will mis-suppress. `TestClaudeArgvIdenticalAcrossPaths` (`test/integration/claude_argv_test.go`) pins both spawn paths to byte-identical argv through the shared exported mappings (`claudeargv.ParamsFromSpawnRequest`, `daraja.ClaudeParamsForRequest` → `SpecFromProto` → `ChildSpec.Argv`); keep `buildClaudeArgv` and `darajaClaudeParams` delegations. `TestMCPInjectedConfigReachesTheAgentControlSurface` (`test/integration`) pins the seam end to end against a real daemon.
...
- **Claude Code's skills-dir plugin contract, verified against the 2.1.263 binary: a plugin's skills load ONLY from `<plugin>/skills/<name>/SKILL.md` — skill directories at the plugin ROOT are invisible, no matter how valid its `.claude-plugin/plugin.json` is.** This is what `pkg/executor/skill_sync.go`'s renderer writes (`<skills>/<ns>/skills/<name>/SKILL.md`, marker + plugin.json at the plugin root; pinned by `TestSyncWritesASkillsDirPlugin`, which also asserts the bare root-level dir does NOT exist). Three more facts learned the expensive way: a bare `skills/<name>/SKILL.md` at the TOP level of the skills dir IS adopted (that is the flat, pre-plugin layout — which is also why the operator-safety property matters); Claude Code keeps its own `<skills-dir>/manifest.json` for sync-owned entries — ABSENT is fine (adopt-all) but PRESENT-and-malformed is fail-closed and skips the whole dir; and a stale tree self-migrates when the renderer changes shape, because `treesIdentical` sees a different file set and the whole-tree replace rebuilds it. Never "fix" the layout by prefixing names — Claude Code derives `rafiki:<name>` from the directory structure, and the fundi renderer must keep agreeing (verified end to end: both child kinds reported identical qualified names).
...
- **`agent_spawn`'s `cwd` does not isolate a child — verify the worktree, never assume it.** Probed 2026-09-07 during the child-budget-mutation plan: a child spawned with `cwd=<worktree>` (even with `workspace: "pinned"`) ran `pwd` in the daemon's cwd on `main`, and a real implementer showed the failure is SPLIT-BRAIN — its file tools wrote into the worktree while its bash verified and committed in the main repo, which it misread as the edit tool silently losing writes; the net effect was a commit directly on `main`, the branch SDD forbids touching. Every dispatch that needs isolation carries an explicit block in the prompt: `cd <worktree> &&` first on EVERY bash invocation, absolute paths under the worktree for edit/read/write, a `pwd` + `git rev-parse --show-toplevel` + `git branch --show-current` check before the first edit (BLOCKED on mismatch), and a re-check before committing (the commit output must name the worktree's branch, never `main`). Seat hygiene rides with this: settled-but-unreaped children still count against the coordinator's live-agent cap (4), so `agent_kill` finished workers before spawning more — a refused spawn naming the cap is the symptom, not a reason to wait.
- **`make check` in a fresh git worktree fails at `test/integration` with every test reporting "daemon never accepted on …/controller.sock".** The worktree has no `.env` (gitignored), so `RAFIKI_TEST_DSN` is unset and the integration harness cannot start daemons — it reads exactly like a daemon regression and cost one implementer a stash-probe to rule out. Source the main checkout's `.env` first (`set -a; . <main-checkout>/.env; set +a`) before gating in any worktree, and treat that signature as missing-env, not a code failure.
...
- **`insights.Scope` is the THIRD zero-value trap carrying behavioural weight, and its per-plane derivations must never be "unified".**
...
- **The MCP agent-control surface (`/mcp`: POST, plus GET for the standalone
  SSE stream and DELETE to close the session) is mounted INSIDE the proxy
  face via `pkg/server.Handler`'s `MCPPath`/`MCP`, and its server is built
  PER REQUEST from `server.IdentityFromContext`.** Three halves are
  load-bearing and all are easy to undo. A `mux.Handle("/mcp", …)` in
  ...
  to make the per-caller binding possible. The third half is the
  session-identity binding in `mcp_face.go`'s `Routes`/`sessionOwnedBy` —
  every request presenting `Mcp-Session-Id` is checked against a
  session→owner map filled at initialize — and it is load-bearing because
  the SDK's own hijack guard is INERT on this mount: `sessInfo.userID` is
...
- **Child rows are shared across every daemon, so destructive writes are gated
  on `ownsChildRow` — which reads the `rafiki/daemon` LABEL, not the
  `daemon_id` column and not `ns_token`.** The gate exists so a daemon cannot
  delete rows belonging to a sibling daemon that is still running. The label
  is stamped by `writeRecordLastStatus` on every persist, so a row the current
  daemon has never re-persisted carries its ORIGINAL pod's id and is refused —
  and the refusal is silent: `Close`/`CloseAllExited` skip the delete and still
  report success. An exited child is never re-persisted, so after a redeploy
  its row is uncloseable forever unless something re-stamps it — which is what
  boot recovery's `AdoptOwnership` (`recoverOne`, foreign-lapsed rows only)
  does: it moves `daemon_id` AND the label together (recovery reads the column,
  Close reads the label — the two gates must never disagree), never `status`
  (row content belongs to the child's own writes), never `ns_token` (it is
  namespace proof for the recorded PID, not an ownership column), and bumps
  `updated_at` so a sibling booting inside `foreignFreshGrace` classifies the
  stamped row foreignLive — the lease, not the stamp, decides who runs the
  child. Closed rows are tombstones (`deleted_at`), filtered by `List` — the
  read that drives recovery — and `Upsert` clears `deleted_at`
  unconditionally: an upsert is the daemon asserting the child is live, which
  is also why a tombstone must never be fed back through Upsert.
...
- **Adding an RPC to `proto/rafiki/v1/control.proto` breaks `pkg/connectapi`
  compilation the moment `make proto` runs.** `connectapi.Server` implements
  the generated `ControlHandler` interface directly (no `UnimplementedControlHandler`
  embed), so a new `rpc` line adds a method to that interface and the tree does
  not compile until the `Server` handler method lands. A proto-only task
  therefore cannot pass `go build ./...` — sequence the proto change and its
  handler into ONE task, or gate the proto task on `go build ./pkg/gen/...`
  alone and carry the whole-tree gate to the handler task (the operator-budget
...
- **The self-kill settlement guard is audience-scoped and causality-based, not shape-based — and claude children never die "clean" when killed.** Verified against the real 2.1.270 binary: claude CATCHES SIGTERM and exits with code 143 and NO signal, so a `signal == "" && exitCode == 0` clean-exit test reads every SIGTERM'd claude as a foreign crash. A daraja-hosted claude child never even reaches the ladder's passive stdin-EOF rung — `darajaStdin.Close` IS the stop request (it issues the executor-side shutdown RPC, SIGTERM, 3s grace), so every agent_kill of one ends signalled within ~seconds. That is why the guard "didn't work for --kind claude": the mark was consumed, the shape test failed, and the coordinator got the "exited" fragment anyway. The rule now is `selfKillDispositionFor(mark, exitCausedByShutdown(res))`: `ShutdownResult.ByShutdown` (pkg/child) records "the shutdown drove this death" — set BEFORE each rung can kill (SIGTERM/SIGKILL, abandon) and before closing a stdin that implements `StdinStopper` — because `handleChildExit` reads the record on `done`'s close and a post-hoc write races it. The suppression is per-audience: a coordinator's agent_kill (`killMark{parent}`) silences its parent's fragment; an MCP caller's (`killMark{mcpUser}` in `userSpawner.Kill`) excludes its own user from `notifyMCPSettled` while the parent still hears (an MCP caller can kill somebody else's worker). A death the kill did NOT cause — fundi's exit-2 panic sentinel, a subprocess crash, a foreign SIGKILL during the passive stdin wait — suppresses nothing. Tests that exercise the MCP fan-out in parallel must use the REAL `mcpSettlements` registry with a unique user id, never the sequential tests' swap-the-global pattern: a swap races every lingering settle goroutine a parallel test leaves behind, and the race detector fails the whole suite (11 unrelated tests) when it fires.
