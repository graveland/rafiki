...
func TestIntegration_CtrlStatus(t *testing.T) {
	...
	st := childstore.New()
	ctrl := NewController(st, stateDir, logsDir, socketPath, nil, nil, nil, t.Context())
...
func TestIntegration_CtrlList(t *testing.T) {
	...
	socketPath := filepath.Join(dir, "c.sock")
	st := childstore.New()
	ctrl := NewController(st, filepath.Join(dir, "state"), filepath.Join(dir, "logs"), socketPath, nil, nil, nil, t.Context())
