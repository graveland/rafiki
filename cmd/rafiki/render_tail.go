...
// non-blank Write call.  It is used by rafiki tail in label-filtered mode to
...
// fans the entire stream to all subscribers — so 'rafiki tail' sees responses
// to other connections' RPC calls (e.g. rafiki-attach's autocomplete
...
func (r *tailRenderer) render(frame []byte) error {
	...
	case protocol.TypeCtrlChildLabeled:
		...
		// In a streaming tail, show ALL labels including rafiki/* — user is
