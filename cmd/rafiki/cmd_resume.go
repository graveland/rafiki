...
Resume from a pi session file (new):
  rafiki resume --pi-session <uuid-or-path>

With --pi-session, rafiki reads the session's .jsonl to extract cwd, model, and
...
After spawning, rafiki attaches the TUI to the new child, identical to 'rafiki create'.
...
func buildResumeFromPiSessionRequest(cmd *cobra.Command, input string) (protocol.SpawnRequest, error) {
	...
	// `rafiki/resumed-from-session=<uuid>`.  Sent here rather than in req.Labels
	// because the daemon (correctly) rejects user-supplied labels in the
	// reserved rafiki/ namespace.
...
// or not that session was ever managed by rafiki.  It reads cwd, model, and
