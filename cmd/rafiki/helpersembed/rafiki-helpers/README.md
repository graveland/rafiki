...
This copies the extension to `~/.pi/agent/extensions/rafiki-helpers/`. Pi auto-discovers it from there for every run (daemon-managed or native).
...
If you start pi with `--no-extensions`, rafiki-helpers will not load and `/reload` will not be available.
