...
// The default kind is "agent", so an unset --kind must offer exactly what the
...
func TestModelSourcesForKind_Agent(t *testing.T) {
	got := modelSourcesForKind("agent")
	if !got[models.SourceOpenRouter] || !got[models.SourceBuiltin] {
		t.Errorf("agent must offer the curated list and OpenRouter slash ids, got %v", got)
	}
	...
	if got[models.SourceUserConfig] {
		t.Error("agent must not offer pi's provider-local ids")
	}
}

func TestModelSourcesForKind_Claude(t *testing.T) {
	got := modelSourcesForKind("claude")
	if !got[models.SourceBuiltin] {
...
func TestModelSourcesForKind_NeverEmpty(t *testing.T) {
	for _, kind := range []string{"", "pi", "agent", "claude", "nonsense"} {
