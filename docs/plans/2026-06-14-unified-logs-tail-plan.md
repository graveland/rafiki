...
- **Backfill `-n N` can show fewer than N events with `--no-deltas`** (found in final review): `Controller.GetRecent` applies the type filter AFTER the limit slice, so `-n 20` takes the last 20 ring entries then strips `message_update`, often yielding fewer visible events. Pre-existing GetRecent semantic; fix would push the filter before the limit (affects `pic recent` too). Deferred.
- **pic-attach widens the 256-frame drop window by one RPC round-trip** (found in final review): the live event iterator is registered in the constructor but only drained after `primeHistory`'s `getRecent` completes, so a child streaming >256 frames within that latency could drop live frames (AsyncQueue silently drops when full). Bounded by one round-trip, low probability. Deferred; fix would pre-buffer during prime or raise the cap for the prime window.
- **`--profile` does not reach backfill** (found in Task 3 review): `fetchBackfill` passes only `Include`/`Exclude` to `ctrl_get_recent`, and `protocol.GetRecentRequest` has no `Profile` field. So `pic tail --profile coarse -n 20` filters the *live* stream by the named profile but backfills the last 20 *unprofiled* events. `--no-deltas`/`--include`/`--exclude` DO reach backfill. Fix (follow-up, out of scope): add a `Profile` field to `GetRecentRequest` and apply it in `Controller.GetRecent`, or expand the named profile to include/exclude client-side before calling `fetchBackfill`.
