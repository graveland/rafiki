...
// mcp__<server>__<tool>, with hyphens in both the server and tool name
// normalized to underscores. Tool results are passed through p.Clip before
...
func ConnectMCP(ctx context.Context, r *Registry, cfg MCPConfig, p OutputPolicy) (func(), error) {
	...
	for name, sc := range cfg.MCPServers {
		...
		if err := registerMCPServerTools(ctx, r, name, session, p); err != nil {
...
func registerMCPServerTools(ctx context.Context, r *Registry, serverName string, session *mcp.ClientSession, p OutputPolicy) error {
	...
	for _, t := range res.Tools {
		...
		r.Register(def, newMCPToolFunc(session, t.Name, def.OfTool.Name, p, &spillCounter))
	}
	return nil
}
...
// normalizeMCPName replaces hyphens with underscores. Anthropic tool names
// must match ^[a-zA-Z0-9_-]{1,128}$ (hyphens are technically legal there),
// but this project's dispatch logic already pattern-matches tool names in
// their underscore form, so both server and tool name segments are
// normalized the same way to keep mcp__ names consistent with the rest of
// the tool surface.
