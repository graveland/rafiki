// Code generated by protoc-gen-connect-go. DO NOT EDIT.
//
// Source: rafiki/admin/v1/admin.proto

package adminpbconnect

import (
	connect "connectrpc.com/connect"
	context "context"
	errors "errors"
	adminpb "go.graveland.dev/rafiki/pkg/adminpb"
	http "net/http"
	strings "strings"
)

// This is a compile-time assertion to ensure that this generated file and the connect package are
// compatible. If you get a compiler error that this constant is not defined, this code was
// generated with a version of connect newer than the one compiled into your binary. You can fix the
// problem by either regenerating this code with an older version of connect or updating the connect
// version compiled into your binary.
const _ = connect.IsAtLeastVersion1_13_0

const (
	// AdminServiceName is the fully-qualified name of the AdminService service.
	AdminServiceName = "rafiki.admin.v1.AdminService"
)

// These constants are the fully-qualified names of the RPCs defined in this package. They're
// exposed at runtime as Spec.Procedure and as the final two segments of the HTTP route.
//
// Note that these are different from the fully-qualified method names used by
// google.golang.org/protobuf/reflect/protoreflect. To convert from these constants to
// reflection-formatted method names, remove the leading slash and convert the remaining slash to a
// period.
const (
	// AdminServiceLaunchProcedure is the fully-qualified name of the AdminService's Launch RPC.
	AdminServiceLaunchProcedure = "/rafiki.admin.v1.AdminService/Launch"
	// AdminServiceReapProcedure is the fully-qualified name of the AdminService's Reap RPC.
	AdminServiceReapProcedure = "/rafiki.admin.v1.AdminService/Reap"
)

// AdminServiceClient is a client for the rafiki.admin.v1.AdminService service.
type AdminServiceClient interface {
	// Launch starts one daraja hosting one child and returns the handles needed
	// to reach and to reap it.
	Launch(context.Context, *connect.Request[adminpb.LaunchRequest]) (*connect.Response[adminpb.LaunchResponse], error)
	// Reap ends a launched daraja and its child: SIGTERM to the process group,
	// wait, then SIGKILL.
	Reap(context.Context, *connect.Request[adminpb.ReapRequest]) (*connect.Response[adminpb.ReapResponse], error)
}

// NewAdminServiceClient constructs a client for the rafiki.admin.v1.AdminService service. By
// default, it uses the Connect protocol with the binary Protobuf Codec, asks for gzipped responses,
// and sends uncompressed requests. To use the gRPC or gRPC-Web protocols, supply the
// connect.WithGRPC() or connect.WithGRPCWeb() options.
//
// The URL supplied here should be the base URL for the Connect or gRPC server (for example,
// http://api.acme.com or https://acme.com/grpc).
func NewAdminServiceClient(httpClient connect.HTTPClient, baseURL string, opts ...connect.ClientOption) AdminServiceClient {
	baseURL = strings.TrimRight(baseURL, "/")
	adminServiceMethods := adminpb.File_rafiki_admin_v1_admin_proto.Services().ByName("AdminService").Methods()
	return &adminServiceClient{
		launch: connect.NewClient[adminpb.LaunchRequest, adminpb.LaunchResponse](
			httpClient,
			baseURL+AdminServiceLaunchProcedure,
			connect.WithSchema(adminServiceMethods.ByName("Launch")),
			connect.WithClientOptions(opts...),
		),
		reap: connect.NewClient[adminpb.ReapRequest, adminpb.ReapResponse](
			httpClient,
			baseURL+AdminServiceReapProcedure,
			connect.WithSchema(adminServiceMethods.ByName("Reap")),
			connect.WithClientOptions(opts...),
		),
	}
}

// adminServiceClient implements AdminServiceClient.
type adminServiceClient struct {
	launch *connect.Client[adminpb.LaunchRequest, adminpb.LaunchResponse]
	reap   *connect.Client[adminpb.ReapRequest, adminpb.ReapResponse]
}

// Launch calls rafiki.admin.v1.AdminService.Launch.
func (c *adminServiceClient) Launch(ctx context.Context, req *connect.Request[adminpb.LaunchRequest]) (*connect.Response[adminpb.LaunchResponse], error) {
	return c.launch.CallUnary(ctx, req)
}

// Reap calls rafiki.admin.v1.AdminService.Reap.
func (c *adminServiceClient) Reap(ctx context.Context, req *connect.Request[adminpb.ReapRequest]) (*connect.Response[adminpb.ReapResponse], error) {
	return c.reap.CallUnary(ctx, req)
}

// AdminServiceHandler is an implementation of the rafiki.admin.v1.AdminService service.
type AdminServiceHandler interface {
	// Launch starts one daraja hosting one child and returns the handles needed
	// to reach and to reap it.
	Launch(context.Context, *connect.Request[adminpb.LaunchRequest]) (*connect.Response[adminpb.LaunchResponse], error)
	// Reap ends a launched daraja and its child: SIGTERM to the process group,
	// wait, then SIGKILL.
	Reap(context.Context, *connect.Request[adminpb.ReapRequest]) (*connect.Response[adminpb.ReapResponse], error)
}

// NewAdminServiceHandler builds an HTTP handler from the service implementation. It returns the
// path on which to mount the handler and the handler itself.
//
// By default, handlers support the Connect, gRPC, and gRPC-Web protocols with the binary Protobuf
// and JSON codecs. They also support gzip compression.
func NewAdminServiceHandler(svc AdminServiceHandler, opts ...connect.HandlerOption) (string, http.Handler) {
	adminServiceMethods := adminpb.File_rafiki_admin_v1_admin_proto.Services().ByName("AdminService").Methods()
	adminServiceLaunchHandler := connect.NewUnaryHandler(
		AdminServiceLaunchProcedure,
		svc.Launch,
		connect.WithSchema(adminServiceMethods.ByName("Launch")),
		connect.WithHandlerOptions(opts...),
	)
	adminServiceReapHandler := connect.NewUnaryHandler(
		AdminServiceReapProcedure,
		svc.Reap,
		connect.WithSchema(adminServiceMethods.ByName("Reap")),
		connect.WithHandlerOptions(opts...),
	)
	return "/rafiki.admin.v1.AdminService/", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case AdminServiceLaunchProcedure:
			adminServiceLaunchHandler.ServeHTTP(w, r)
		case AdminServiceReapProcedure:
			adminServiceReapHandler.ServeHTTP(w, r)
		default:
			http.NotFound(w, r)
		}
	})
}

// UnimplementedAdminServiceHandler returns CodeUnimplemented from all methods.
type UnimplementedAdminServiceHandler struct{}

func (UnimplementedAdminServiceHandler) Launch(context.Context, *connect.Request[adminpb.LaunchRequest]) (*connect.Response[adminpb.LaunchResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("rafiki.admin.v1.AdminService.Launch is not implemented"))
}

func (UnimplementedAdminServiceHandler) Reap(context.Context, *connect.Request[adminpb.ReapRequest]) (*connect.Response[adminpb.ReapResponse], error) {
	return nil, connect.NewError(connect.CodeUnimplemented, errors.New("rafiki.admin.v1.AdminService.Reap is not implemented"))
}
