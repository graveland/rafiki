...
func TestServer_AcceptsAndEchoes(t *testing.T) {
	...
	srv, err := control.Listen(sockPath, control.FuncHandler(func(_ control.Connection, frame []byte) []byte {
...
func TestServer_LiveSocketRejected(t *testing.T) {
	...
	noHandler := control.FuncHandler(func(control.Connection, []byte) []byte { return nil })

	srv1, err := control.Listen(sockPath, noHandler)
	if err != nil {
		t.Fatal(err)
	}
	defer srv1.Close()
	...
	_, err = control.Listen(sockPath, noHandler)
	if err == nil {
...
func TestServer_Broadcast(t *testing.T) {
	...
	noop := control.FuncHandler(func(_ control.Connection, _ []byte) []byte { return nil })
	srv, err := control.Listen(sockPath, noop)
	if err != nil {
		t.Fatal(err)
	}
	defer srv.Close()
...
func TestServer_StaleSocketUnlinked(t *testing.T) {
	...
	noopHandler := control.FuncHandler(func(control.Connection, []byte) []byte { return nil })

	// First listener.
	srv1, err := control.Listen(sockPath, noopHandler)
	if err != nil {
		t.Fatal(err)
	}
	...
	srv2, err := control.Listen(sockPath, noopHandler)
	if err != nil {
