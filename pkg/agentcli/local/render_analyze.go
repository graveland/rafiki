...
// table renders one table section through agentcli.WriteTable, stopping at
// the first error like every other errWriter method.
func (e *errWriter) table(title string, numeric []int, header []string, rows [][]string) {
	if e.err != nil {
		return
	}
	e.err = agentcli.WriteTable(e.w, title, numeric, header, rows)
}
...
func RenderFindings(w io.Writer, rows []store.FindingRow) error {
	...
	ew := &errWriter{w: w}
	ew.table("Findings", nil, header, out)
	return ew.err
}
