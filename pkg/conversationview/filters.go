...
// This deliberately duplicates the validation in insightstypes.Path.validate
...
// BindStatsFilter maps FilterVals to insightstypes.StatsFilter, parsing time strings
// and validating the Path enum. Since/Until errors are wrapped with context.
func BindStatsFilter(v FilterVals) (insightstypes.StatsFilter, error) {
	// Validate Path enum early to avoid DB round-trip on invalid input
	if err := validatePath(v.Path); err != nil {
		return insightstypes.StatsFilter{}, err
	}

	since, err := ParseTime(v.Since)
	if err != nil {
		return insightstypes.StatsFilter{}, fmt.Errorf("since: %w", err)
	}

	until, err := ParseTime(v.Until)
	if err != nil {
		return insightstypes.StatsFilter{}, fmt.Errorf("until: %w", err)
	}

	return insightstypes.StatsFilter{
		Since:   since,
		Until:   until,
		Owner:   v.Owner,
		Persona: v.Persona,
		Source:  v.Source,
		Model:   v.Model,
		Path:    insightstypes.Path(v.Path),
	}, nil
}

// BindSearchFilter maps FilterVals to insightstypes.SearchFilter, parsing time strings
// and validating the Path enum. Since/Until errors are wrapped with context.
func BindSearchFilter(v FilterVals) (insightstypes.SearchFilter, error) {
	// Validate Path enum early to avoid DB round-trip on invalid input
	if err := validatePath(v.Path); err != nil {
		return insightstypes.SearchFilter{}, err
	}

	since, err := ParseTime(v.Since)
	if err != nil {
		return insightstypes.SearchFilter{}, fmt.Errorf("since: %w", err)
	}

	until, err := ParseTime(v.Until)
	if err != nil {
		return insightstypes.SearchFilter{}, fmt.Errorf("until: %w", err)
	}

	return insightstypes.SearchFilter{
		Since:             since,
		Until:             until,
		Owner:             v.Owner,
		Persona:           v.Persona,
