...
func RenderProgress(w io.Writer, p *Progress) error {
	if p == nil {
		return nil
	}
	ew := &errWriter{w: w}
...
// RenderAnalyzeSummary renders the terminal Summary of an Analyze run: a
// ranked-findings table, a totals line, and a "remaining" note when the
// run's limit left eligible conversations untouched.
func RenderAnalyzeSummary(w io.Writer, s *Summary) error {
	if s == nil {
...
// *insights.Transcript renders via RenderTranscriptMD.
...
func WriteSkillEdits(outDir string, s *Summary) ([]string, error) {
	if s == nil {
		return nil, nil
	}
